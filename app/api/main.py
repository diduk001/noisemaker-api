...
def generate_noises(n, width, height):
    tensors = list()
    for i in range(n):
        tensors.append(random.choice((noisemaker.generators.basic([random.randint(1,
                       20), random.randint(1, 20)], [width, height,
                       3]),
                       noisemaker.generators.multires([random.randint(1,
                       20), random.randint(1, 20)], [width, height,
                       3]))))
    return tensors
...
def mix_all(tensors):
    ...
    for i in range(1, len(tensors)):
        cur = random.choice((noisemaker.effects.morph(cur, tensors[i],
                            random.uniform(0, 1), random.randint(1, 3),
                            spline_order=random.randint(1, 2)),
                            noisemaker.effects.blend(cur, tensors[i],
                            random.uniform(0, 1)),
                            noisemaker.effects.blend_cosine(cur,
                            tensors[i], random.uniform(0, 1))))
    return cur
...
def choose(tensors, mode, index=0):
    ...
    if mode == 0:
        ...
        chosens = random.choices(tensors, k=random.randint(2,
                                 len(tensors)))
...
def all_random(width, height):
    ...
    tensor_with_effects = put_effects(chosen_tensor, random.randint(0,
            2))
    img = render(tensor_with_effects, width, height)

    pic_name = os.path.abspath(os.path.join('results',
...
def by_modes(width, height, n, mode1, index, mode2, order):
    ...
    tensor_with_effects = put_effects(chosen_tensor, mode=mode2,
            order=order)

    img = render(tensor_with_effects, width, height)

    pic_name = os.path.join('results',
                            datetime.now().strftime('%Y-%m-%d %H:%M:%S'
                            ) + '.png')
    img.save(pic_name, 'PNG')
